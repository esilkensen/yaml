...
(define scanner+c%
  (class/c
   ...
   [peek-token (->m (or/c token? #f))]
   [get-token (->m (or/c token? #f))]))
...
(define (scan [in (current-input-port)])
  ...
  (let loop ([tokens '()])
    (if (token? (send scanner peek-token))
        (loop (cons (send scanner get-token) tokens))
        (reverse tokens))))
...
(define scanner%
  ...
  ;;; Public methods.
    ...
    (define/public (check-token? . choices)
  ...
  ;;; Checkers.
    ...
    ;;        (mark mark String -> anchor-token)) ->
    ;;     (U alias-token anchor-token)))
    ...
    (define (scan-flow-scalar style)
      ...
      (let ([double (equal? #\" style)]
        ...
        (let ([chunks (scan-flow-scalar-non-spaces double)])
          ...
          (let ([end-mark (get-mark)])
            (scalar-token
             start-mark end-mark (list->string chunks) #f style)))))
           ...
           (while #t
             ...
             (let ([pre (prefix 3)])
               ...
               (if (and (char? (peek))
                        (string-index "\r\n\x85\u2028\u2029" (peek)))
                   (set! chunks
                         (append chunks (string->list (scan-line-break))))
                   (break (void)))))))
        chunks))
