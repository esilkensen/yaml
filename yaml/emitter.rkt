...
(define emitter%
  (class object%
    ...
    (define (increase-indent [flow #f] [indentless #f])
      (append! indents (list current-indent))
      (if (eq? #f current-indent)
          (set! current-indent (if flow best-indent 0))
          (unless indentless
            (set! current-indent (+ current-indent best-indent)))))
    ...
    (define (write-indent)
      (let ([this-indent (or current-indent 0)])
        (when (or (not indention) (> column this-indent)
                  (and (= column this-indent) (not whitespace)))
          (write-line-break))
        (when (< column this-indent)
          (let ([n (- this-indent column)])
            (set! whitespace #t)
            (set! column this-indent)
                  ...
                  (cond
                    ...
                    [else
                     (let ([hex (number->string (char->integer ch) 16)])
                       (if (= 2 (string-length hex))
                           (set! data
                                 (format "\\U000000~a" (string-upcase hex)))
                           (set! data
                                 (format "\\U0000~a" (string-upcase hex)))))])
                  (set! column (+ column (string-length data)))
                  (display data out)
                  (set! start (add1 end)))))
...
(module+ test
  ...
  (for ([(test-file check-file) (test-files #".emit")])
    (test-case (path->string check-file)
      (let* ([out (open-output-string)]
             [emitter (new emitter% [out out])])
        (for ([event (parse-file test-file)])
          (send emitter emit event))
        (check-equal?
  ...
  (test-case "expect-stream-start"
    ...
    (send emitter emit (second foo-seq))
    (check-exn
     #rx"expected stream-start"
     (λ () (send emitter emit (third foo-seq)))))
  
  (test-case "expect-nothing"
    (define emitter (new emitter%))
    (send emitter emit (first foo-seq))
    (send emitter emit (last foo-seq))
    (check-exn
     #rx"expected nothing"
     (λ () (send emitter emit (last foo-seq)))))
  ...
  (test-case "expect-document-start"
    ...
    (check-exn
     ...
     (λ () (send emitter emit (sixth foo-seq)))))
  
  (test-case "expect-document-end"
    (define emitter (new emitter% [out (open-output-string)]))
    (for ([e (drop-right foo-seq 2)]) (send emitter emit e))
    (check-exn
  ...
  (test-case "prepare-version"
    ...
    (send emitter emit (first evs))
    (send emitter emit (second evs))
    (check-exn
     #rx"unsupported YAML version: 2.0"
     (λ () (send emitter emit (third evs)))))
